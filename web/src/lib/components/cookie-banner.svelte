<script lang="ts">
	...
	const localStoredCookieTypes = browser
		? (JSON.parse(localStorage.getItem("cookieTypes") as string) as string[])
		: undefined;
...
<div class="group fixed top-0 left-0 z-[60] h-screen w-full overflow-hidden">
	<div class="h-[7.5vh] w-full"></div>

	<!-- Banner content -->
	<div class="relative flex h-max w-full flex-col gap-6 bg-muted g-px py-7 shadow-lg">
		<button
		...
		<div class="flex flex-col gap-6 md:flex-row md:items-start">
			...
			<section class="flex flex-col gap-6">
				...
				{#if showCookies}
					<div class="flex flex-col gap-4">
						{#each cookies as cookie (cookie.value)}
							...
							<Label
								for={cookie.value}
								class="flex cursor-pointer items-start gap-2 disabled:cursor-default"
							>
								<Checkbox
				...
				<!-- Action buttons -->
				<div class="**:pointer-events- flex flex-col gap-4 md:flex-row">
					<Button class="cursor-pointer" onclick={handleCookies}>
						{showCookies ? "Save and Close" : "Customize settings"}</Button
					>
					<Button
