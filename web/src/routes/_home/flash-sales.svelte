<script lang="ts">
	...
	const flashSales = $derived(flashSalesData as FlashSaleI);

	const imageUrl = $derived(flashSales?.product?.images[0]?.url ?? "");
	const imageAlt = $derived(flashSales?.product?.images[0]?.alternativeText ?? "");
</script>
